...
/// Start backend + frontend servers. Called both at startup and via IPC.
fn spawn_sidecars(state: &SidecarProcesses) -> Result<(), String> {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            // Auto-start backend + frontend on app launch
            let state = app.state::<SidecarProcesses>();
            if let Err(e) = spawn_sidecars(&state) {
                eprintln!("Failed to start sidecars: {}", e);
            }
            Ok(())
        })
